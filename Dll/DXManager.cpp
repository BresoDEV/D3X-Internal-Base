...
void DXManager::DrawTextA(const char* str, int x, int y, DWORD color, int f)
{
	...
	{
	case 0:
		Texto->DrawText(nullptr, str, -1, &pText, DT_NOCLIP | DT_RIGHT, color);
		break;

	case 1:
		Texto->DrawText(nullptr, str, -1, &pText, DT_NOCLIP | DT_CENTER, color);
		break;

	case 2:
		Texto->DrawText(nullptr, str, -1, &pText, DT_NOCLIP | DT_LEFT, color);
		break;
	}
}
...
HRESULT WINAPI OnEndSceneFunc(LPDIRECT3DDEVICE9 pDevice)
{
	...
	if (!Texto)
		D3DXCreateFontA(pDevice, 14, 0, FW_BOLD, 0, 0, DEFAULT_CHARSET, OUT_TT_ONLY_PRECIS, PROOF_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Texto", &Texto);
